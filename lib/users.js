...
CREATE TABLE IF NOT EXISTS users (
  ...
  branch_id INTEGER,                           -- الفرع داخل الشركة (NULL = كل الفروع)
...
function createUser(data) {
  ...
  let branchId = null;
  if (role !== 'platform' && data.branch_id !== undefined && data.branch_id !== null && data.branch_id !== '') {
    branchId = Number(data.branch_id);
    if (!Number.isFinite(branchId)) throw new Error('معرّف الفرع غير صالح');
  }
  ...
  const info = masterDb.prepare(`
    INSERT INTO users (username, password_hash, role, company_id, branch_id, permissions, is_active, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(username, `${salt}:${hashPassword(password, salt)}`, role, companyId, branchId, perms, data.is_active === false ? 0 : 1, now, now);
...
function updateUser(id, data) {
  ...
  let branchId = u.branch_id != null ? u.branch_id : null;
  if (data.branch_id !== undefined) {
    branchId = (data.branch_id === null || data.branch_id === '') ? null : Number(data.branch_id);
    if (branchId !== null && !Number.isFinite(branchId)) throw new Error('معرّف الفرع غير صالح');
  }

  masterDb.prepare(`
    UPDATE users SET username = ?, role = ?, branch_id = ?, permissions = ?, is_active = ?, updated_at = ? ${passSql}
    WHERE id = ?
  `).run(username, role, branchId, perms, isActive, new Date().toISOString(), ...params, id);
...
// ترحيل آمن: إضافة عمود الفرع لقواعد المستخدمين القديمة
(function migrateUserBranch() {
  const cols = masterDb.prepare('PRAGMA table_info(users)').all().map(c => c.name);
  if (!cols.includes('branch_id')) masterDb.exec('ALTER TABLE users ADD COLUMN branch_id INTEGER');
})();
