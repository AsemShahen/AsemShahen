...
async function createInvoice(db, { kind, party_id, date, lines, discount = 0, vat_rate, payment_method = 'cash', paid_amount, due_date, notes, fiscal_year_id, company, warehouse_id, branch_id }) {
  ...
  const tx = db.transaction(() => {
    ...
    const info = db.prepare(`
      INSERT INTO invoices (invoice_no, kind, party_id, date, sub_total, discount, vat, total, vat_rate,
        payment_method, status, paid_amount, due_date, notes, fiscal_year_id, branch_id, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(invoiceNo, kind, party_id, date, subTotal, totalDiscount, vat, total, vatRate,
      payment_method, status, paid, due_date || null, notes || '', fiscal_year_id, bid, new Date().toISOString());
    ...
    postInvoiceJournal(db, { invoiceId, kind, party_id, date, lines: cleanLines, subTotal, discount: totalDiscount, vat, total, vatRate, payment_method, paid, status, fiscal_year_id, branch_id: bid });
    return invoiceId;
  });
...
function postInvoiceJournal(db, { invoiceId, kind, party_id, date, lines, subTotal, discount, vat, total, vatRate, payment_method, paid, status, fiscal_year_id, branch_id }) {
  ...
  if (kind === 'sale') {
    ...
    createJournalEntry(db, {
      ...
      fiscal_year_id, branch_id, lines: journalLines
    });
  } else {
    // شراء
...
function recordPayment(db, { invoiceId, amount, date, method, fiscal_year_id }) {
  ...
  const tx = db.transaction(() => {
    ...
    if (inv.kind === 'sale') {
      ...
      createJournalEntry(db, { date, description: `سداد عميل - فاتورة ${inv.invoice_no}`, ref_type: 'payment', ref_id: invoiceId, fiscal_year_id, branch_id: inv.branch_id, lines });
    } else {
      const payableAcct = getDefaultAccount(db, '2101');
      const lines = [
      ...
      createJournalEntry(db, { date, description: `سداد مورد - فاتورة ${inv.invoice_no}`, ref_type: 'payment', ref_id: invoiceId, fiscal_year_id, branch_id: inv.branch_id, lines });
    }
  });
