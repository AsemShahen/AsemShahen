...
// ---------- قيود اليومية ----------
function createJournalEntry(db, { date, description, ref_type = 'manual', ref_id = null, fiscal_year_id, lines, branch_id = null, is_closing = 0, is_opening = 0 }) {
  ...
  const tx = db.transaction(() => {
    const info = db.prepare(`
      INSERT INTO journal_entries (entry_no, date, description, ref_type, ref_id, fiscal_year_id, branch_id, is_closing, is_opening, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(entryNo, date, description || '', ref_type, ref_id, fiscal_year_id, branch_id, is_closing, is_opening, new Date().toISOString());
