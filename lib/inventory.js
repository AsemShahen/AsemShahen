...
function listWarehouses(db, { branch_id } = {}) {
  let sql = `
  ...
  const params = [];
  if (branch_id) { sql += ` WHERE w.branch_id = ?`; params.push(Number(branch_id)); }
  sql += ` ORDER BY w.is_default DESC, w.name`;
  return db.prepare(sql).all(...params);
}

function getWarehouse(db, id) {
  return db.prepare(`SELECT w.*, b.name AS branch_name FROM warehouses w
    LEFT JOIN branches b ON b.id = w.branch_id WHERE w.id = ?`).get(id);
}

function createWarehouse(db, data) {
  const name = String(data.name || '').trim();
  if (!name) throw new Error('اسم المستودع مطلوب');
  const branchId = branches.resolveBranchId(db, data.branch_id);
  const info = db.prepare(`
    INSERT INTO warehouses (code, name, location, notes, branch_id, is_default, is_active, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(nextWarehouseNo(db), name, data.location || '', data.notes || '', branchId,
...
function updateWarehouse(db, id, data) {
  ...
  if (!w) return null;
  const branchId = data.branch_id !== undefined ? branches.resolveBranchId(db, data.branch_id) : w.branch_id;
  db.prepare(`UPDATE warehouses SET name=?, location=?, notes=?, branch_id=?, is_active=? WHERE id=?`)
    .run((data.name || w.name), data.location !== undefined ? data.location : w.location,
      data.notes !== undefined ? data.notes : w.notes, branchId,
...
function listProducts(db, { search, includeInactive = false, manufacturedOnly = false } = {}) {
  let sql = `SELECT p.*,
  ...
  if (manufacturedOnly) sql += ` AND EXISTS (SELECT 1 FROM recipes r WHERE r.product_id = p.id AND r.is_active = 1)`;
  if (search) {
...
function findByBarcode(db, barcode, { manufacturedOnly = false } = {}) {
...
function createProduct(db, data) {
  ...
  const info = db.prepare(`
    INSERT INTO products (code, name, barcode, category, unit, description, image, purchase_price, sale_price,
      min_stock, vat_applicable, sale_account, purchase_account, cogs_account, inventory_account, is_active, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(code, name, barcode, data.category || '', data.unit || '', data.description || '', data.image || '',
...
function updateProduct(db, id, data) {
  ...
  db.prepare(`UPDATE products SET name=?, barcode=?, category=?, unit=?, description=?, image=?, purchase_price=?,
    ...
    .run((data.name || p.name), barcode,
      ...
      data.image !== undefined ? data.image : p.image,
...
function listMovements(db, { productId, warehouseId, branch_id, limit = 300 } = {}) {
  ...
  if (branch_id) { sql += ` AND w.branch_id = ?`; params.push(Number(branch_id)); }
